...
static ERRORTYPE createVoChn(SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData)
{
   ...
   //enable vo layer
    ...
    pDemux2Vdec2VoData->mVoLayerAttr.stDispRect.Width = 240;
    pDemux2Vdec2VoData->mVoLayerAttr.stDispRect.Height = 320;
	...
	//spPubAttr.enIntfSync = VO_OUTPUT_1080P50;
...
static void MenuCtrl(int val)
{
    ...
    if( val )
    {
        write(fifoFd, "P[DE] PASS", 30);
    }
    else
    {
        write(fifoFd, "F[DE] FAIL", 30);
    }
}
...
//        AW_MPI_VO_RemoveOutsideVideoLayer(pDemux2Vdec2VoData->mUILayer);
    ...
    AW_MPI_VO_OpenVideoLayer(pDemux2Vdec2VoData->mUILayer); /* open ui layer. */
    AW_MPI_VO_AddOutsideVideoLayer(pDemux2Vdec2VoData->mUILayer);
    //AW_MPI_VO_RemoveOutsideVideoLayer(pDemux2Vdec2VoData->mUILayer);
    /* disalbe vo dev */
    AW_MPI_VO_Disable(pDemux2Vdec2VoData->mVoDev);
...
//    MenuCtrl(m_bTestSuccess);
    ...
    if (result == 0) {
       alogd("sample_demux2vdec2vo PASS!\n");
       fwrite("P[DE] PASS",15,1,fd);
    }else{
       alogd("sample_demux2vdec2vo FAIL!\n");
       fwrite("F[DE] FAIL",15,1,fd);
    }
   fclose(fd);
   return result;
}
