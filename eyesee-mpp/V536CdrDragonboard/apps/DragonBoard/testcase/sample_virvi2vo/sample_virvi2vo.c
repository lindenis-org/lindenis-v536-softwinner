...
int main(int argc __attribute__((__unused__)), char *argv[] __attribute__((__unused__)))
{
    ...
    fd = fopen(FIFO_CSI_DEV,"wb+");
    aloge("load sample_virvi2volcd.conf");
...
#define ISP_RUN 1
 ...
 /*   VO_PUB_ATTR_S spPubAttr;
    ...
    usleep(900 * 1000);
    AW_MPI_VO_SetVideoLayerPriority(stContext.mUILayer, 8);
    stAlpha.mAlphaMode = 0;
    stAlpha.mAlphaValue = 150;
    AW_MPI_VO_SetVideoLayerAlpha(stContext.mUILayer, &stAlpha);
    ...
    */
    ...
    {
        if (result == 0) {
           char str_pass[] = "P[CSI] PASS";
           printf("sample_virvi2vo pass!\n");
           fwrite(str_pass,1,strlen(str_pass),fd);
           aloge("lcdtest:csi pass");
           fclose(fd);
        }else{
           char str_fail[] = "F[CSI] FAIL";
           printf("sample_virvi2vo fail!\n");
           fwrite(str_fail,1,strlen(str_fail),fd);
           aloge("lcdtest:csi fail");
           fclose(fd);
           goto exit;
        }
