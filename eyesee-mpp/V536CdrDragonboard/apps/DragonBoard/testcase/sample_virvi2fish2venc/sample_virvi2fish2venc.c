...
int aw_vipp_creat(VI_DEV ViDev, VI_ATTR_S *pstAttr)
{
    ...
    if (ViDev == 0 || ViDev == 2) {
        isp_dev = 1;
    } else if(ViDev == 1 || ViDev == 3) {
        isp_dev = 0;
    }
    AW_MPI_ISP_Run(isp_dev); // 3A ini
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    int result = 0;
    int AutoTestCount = 0;
    FILE *ise_fd = NULL,*ve_fd = NULL;
    ise_fd = fopen(FIFO_ISE_DEV, "wb+");
    ve_fd  = fopen(FIFO_VE_DEV, "wb+");
...
sys_exit:
    ...
    if(ret == 0){
        aloge("ise ve test fish write pass");
        fwrite("P[VE] PASS",15,1,ve_fd);
        fwrite("P[ISE] PASS",15,1,ise_fd);
    }
    return 0;
_exit:
        return result;
