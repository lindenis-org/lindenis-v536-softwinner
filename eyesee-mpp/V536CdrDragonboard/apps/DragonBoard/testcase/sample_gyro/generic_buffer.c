        ...
        for (k = 0; k < gyro_attr->arry_elems; k++) {
            ...
            switch (gyro_attr->iio_chn[k].bytes) {
                ...
                case 8: /*mpu6500 : x,y,z,time*/
                    print8byte(&gyro_pkt.time_stamp,
                        *(uint64_t *)(read_tmp + gyro_attr->iio_chn[k].location),
                           &gyro_attr->iio_chn[k]);
//                    printf("%llu\n", stGyroPkt.dTimeStamp);
                    gyro_pkt.time_stamp = gyro_pkt.time_stamp / 1000; // ns -> us
//                    printf("%llu\t\t", stGyroPkt.dTimeStamp);
//                    if (stGyroPkt.dTimeStamp)
//                        printf("%llu\r\n", stGyroPkt.dTimeStamp);
...
//                    else if (0 != gyro_pkt.time_stamp && gyro_pkt.time_stamp < g_gyro_chn_mgr.last_pkt_timestamp)
//                        gyro_pkt.time_stamp = (g_gyro_chn_mgr.last_pkt_timestamp + 1000000 / gyro_attr->sample_freq);
...
//            printf("Ring buffer[%d].\r\n", gelem_cur->idx);
            ...
            if (ring_buffer_full(gelem_cur->rb_hd)) {
                struct gyro_pkt pkt_out;
                static unsigned int fuu_cnt = 0;
                if (0 == (fuu_cnt % g_gyro_chn_mgr.gyro_attr.sample_freq))
                    printf("Ring buffer[%d] full, drop the oldest one gyro data.\r\n", gelem_cur->idx);
                fuu_cnt++;
                ring_buffer_out(gelem_cur->rb_hd, &pkt_out, 1);
            }
...
int main(int argc, char *argv[])
{
    ...
    ret = gyro_ins->open(gyro_ins, &gyro_attr);
    if(ret < 0){
        printf("gyro_ins open failed!!!!!\r\n");

        goto exit_;
    }

    ret = gyro_ins1->open(gyro_ins1, &gyro_attr);
    if(ret < 0){
        printf("gyro_ins1 open failed!!!!!\r\n");
        goto exit_1;
    }

    ret = gyro_ins2->open(gyro_ins2, &gyro_attr);
    if(ret < 0){
        printf("gyro_ins2 open failed!!!!!\r\n");
        goto exit_2;
    }
...
exit_:
    ...
    if(ret == 0){
        printf("sample_gyro exit!\n");
        fwrite("P[GYRO] PASS",15,1,fd);
        printf("sample_gyro PASS!\n");
    }else{
        fwrite("F[GYRO] FAIL",15,1,fd);
        printf("sample_gyro FAIL!\n");
    }
    fclose(fd);
    return 0;
}
