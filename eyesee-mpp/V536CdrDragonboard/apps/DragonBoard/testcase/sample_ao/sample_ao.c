...
_exit:
    ...
    if (result == 0) {
        ...
        fwrite("P[SPK] PASS",15,1,fd);
        alogd("sample_ao PASS!\n");
    }else{
        fwrite("F[SPK] FAIL",15,1,fd);
        alogd("sample_ao FAIL!\n");
    }
    fclose(fd);
    return result;
}
