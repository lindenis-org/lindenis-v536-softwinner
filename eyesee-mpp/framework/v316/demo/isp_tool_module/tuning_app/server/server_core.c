...
void *sock_handle_heart_jump_thread(void *params)
{
	...
	close(sock_fd);
	g_thread_status &= ~TH_STATUS_HEART_JUMP;
	//NOTIFY_ALL_STOPS();//zhubin modify
	LOG("%s: exits - %d\n", __FUNCTION__, sock_fd);
...
void *sock_handle_venc_tuning_thread(void *params)
{
	int ret = 0;
	sock_packet comm_packet;
	int sock_fd = (int)params;
	int VeChn = 0, cfg_length = 0, type = 0;
	HW_U8 group_id = 0;
	HW_U32 cfg_ids = 0;
	unsigned char *buffer = (unsigned char *)malloc(sizeof(char)*500); // enough memory
	int buf_length = 0;
	int timeout_times = 0;

	LOG("%s: starts - %d\n", __FUNCTION__, sock_fd);
	g_thread_status |= TH_STATUS_VENC_TUNING;
	...
	while (1) {
		...
		if (SHOULD_CLOSE_SOCK(ret)) {
			...
			cfg_ids = ((comm_packet.cmd_ids[3] << 24) | (comm_packet.cmd_ids[4] << 16)
						| (comm_packet.cmd_ids[5] << 8) | comm_packet.cmd_ids[6]);
			cfg_length = ntohl(comm_packet.data_length);
			VeChn = ntohl(comm_packet.reserved[0]);
			if (SOCK_CMD_VENC_GET == type) {
				LOG("%s: get venc(vechn %d) - group:%02x, cfgs:%08x, length:%d\n", __FUNCTION__, VeChn, group_id, cfg_ids, cfg_length);
				ret = venc_get_cfg(VeChn, group_id, cfg_ids, (void *)buffer);
				if (ret > 0) { // get config ok
					buf_length = ret;
					comm_packet.data_length = htonl(buf_length);
					convert_tuning_cfg_to_network(group_id, cfg_ids, buffer);
					...
					LOG("%s: failed to get config(Vechn %d)\n", __FUNCTION__, VeChn);
					comm_packet.ret = htonl(SOCK_CMD_RET_FAILED);
					ret = sock_write(sock_fd, (const void *)&comm_packet, sizeof(comm_packet), SOCK_DEFAULT_TIMEOUT);
				}
			} else if (SOCK_CMD_VENC_SET == type) {
				LOG("%s: set cfg(VeChn %d) - group:%02x, cfgs:%08x, length:%d\n", __FUNCTION__, VeChn, group_id, cfg_ids, cfg_length);
				...
				if (sizeof(comm_packet) == ret) {
					...
					if (ret == cfg_length) {
						convert_tuning_cfg_to_local(group_id, cfg_ids, buffer);
						//printf("%s: ready to set Vechn cfg\n", __FUNCTION__);
						ret = venc_set_cfg(VeChn, group_id, cfg_ids, (void *)buffer);
						//printf("%s: set Vechn cfg done\n", __FUNCTION__);
						if (ret == 0) {  // set config ok
							comm_packet.ret = htonl(SOCK_CMD_RET_OK);
						} else {
							comm_packet.ret = htonl(SOCK_CMD_RET_FAILED);
							LOG("%s: failed to set config(vechn %d)\n", __FUNCTION__, VeChn);
						}
						ret = sock_write(sock_fd, (const void *)&comm_packet, sizeof(comm_packet), SOCK_DEFAULT_TIMEOUT);
					} else {
						LOG("%s: failed to read configs from client(vechn %d, %d)\n", __FUNCTION__, VeChn, ret);
					}
				} else {
					LOG("%s: failed to write set config reply(vechn %d, %d)\n", __FUNCTION__, VeChn, ret);
				}
			} 
		} else {
			if (ETIMEDOUT == errno) {
