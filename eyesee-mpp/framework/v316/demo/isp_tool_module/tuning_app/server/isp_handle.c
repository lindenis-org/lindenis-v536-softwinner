...
static HW_U64 htonl64(HW_U64 u64Host)
{

#ifdef PLATFORM_LITTLE_ENDIAN
    HW_U64 result = 0;
    
    HW_U32 u32HostLow = u64Host & 0xffffffff;
    HW_U32 u32HostHight = (u64Host >> 32) & 0xffffffff;

    result = htonl(u32HostLow);

    result <<= 32; // big ending

    result |= htonl(u32HostHight);
...
static HW_U64 ntohl64(HW_U64 u64Net)
{
    ...
    HW_U32 u32NetLow = u64Net & 0xffffffff;
    HW_U32 u32NetHight = (u64Net >> 32) & 0xffffffff;
    
    result = ntohl(u32NetLow);

    result <<= 32;

    result |= ntohl(u32NetHight); 
...
void ntoh_3a_info(void *stat_info, int type)
{
	if (stat_info) {
		...
		if (SOCK_CMD_STAT_AE == type) {
			struct isp_ae_stats_s *ae_stat = (struct isp_ae_stats_s *)stat_info;
			ae_stat->win_pix_n = ntohl64(ae_stat->win_pix_n);
			// accum_r
			ptr_u64 = &ae_stat->accum_r[0][0];
			for (i = 0; i < ISP_AE_ROW * ISP_AE_COL; i++, ptr_u64++) {
				*ptr_u64 = ntohl64(*ptr_u64);
			}
			// accum_g
			ptr_u64 = &ae_stat->accum_g[0][0];
			for (i = 0; i < ISP_AE_ROW * ISP_AE_COL; i++, ptr_u64++) {
