...
static void *Vipp2VencThread(void *pArg)
{
  ...
  #if (SUPPORT_EVE!=0)
    AW_AI_EVE_EVENT_RESULT_S stEveResult;
  #endif
    while(1) {
//      pthread_testcancel();
        if (g_bSampleExit) {
      ...
      #if (SUPPORT_EVE!=0)
        if (pConfig->bUseEve) {
            iRet = AW_MPI_VI_GetFrame(iVippDev, iVippChn, &stFrameInfo, 60);
            if (iRet < 0) {
                aloge("Vipp2VencThread get one frame failed!!\n");
                goto process;
            }

            /* draw rectangulr */
    //		pthread_mutex_lock(&pUVCInfo->mEveFaceResLock);
            memcpy(&stEveResult, &pUVCInfo->stEveRes, sizeof(AW_AI_EVE_EVENT_RESULT_S));
    //		pthread_mutex_unlock(&pUVCInfo->mEveFaceResLock);
            for (i = 0; i < stEveResult.sTarget.s32TargetNum; i++)
            {
                //DB_PRT(" TargetNum:%d  (%d-%d) (%d-%d)\n", pstEveResult.sTarget.s32TargetNum, sx, sy, ex, ey);
                sx = (stFrameInfo.VFrame.mWidth  * stEveResult.sTarget.astTargets[i].stRect.s16Left) / 640;
                sy = (stFrameInfo.VFrame.mHeight * stEveResult.sTarget.astTargets[i].stRect.s16Top)  / 360;
                ex = (stFrameInfo.VFrame.mWidth  * stEveResult.sTarget.astTargets[i].stRect.s16Right)  / 640; 
                ey = (stFrameInfo.VFrame.mHeight * stEveResult.sTarget.astTargets[i].stRect.s16Bottom) / 360; 
                draw_rectangle_nv21((unsigned char *)stFrameInfo.VFrame.mpVirAddr[0], (unsigned char *)stFrameInfo.VFrame.mpVirAddr[1],
                            stFrameInfo.VFrame.mWidth, stFrameInfo.VFrame.mHeight, 3,
                            sx, sy, ex, ey);
            #if 1
                for(j = 0; j < stEveResult.sTarget.astTargets[i].stTraject.length; j++)
                {
                    sx = stFrameInfo.VFrame.mWidth*(stEveResult.sTarget.astTargets[i].stTraject.points[j].s16X - 1)/640;
                    ex = stFrameInfo.VFrame.mWidth*(stEveResult.sTarget.astTargets[i].stTraject.points[j].s16X + 1)/640;
                    sy = stFrameInfo.VFrame.mHeight*(stEveResult.sTarget.astTargets[i].stTraject.points[j].s16Y - 1)/360;
                    ey = stFrameInfo.VFrame.mHeight*(stEveResult.sTarget.astTargets[i].stTraject.points[j].s16Y + 1)/360;
                    draw_rectangle_nv21((unsigned char *)stFrameInfo.VFrame.mpVirAddr[0], (unsigned char *)stFrameInfo.VFrame.mpVirAddr[1],
                            stFrameInfo.VFrame.mWidth, stFrameInfo.VFrame.mHeight, 3,
                            sx, sy, ex, ey);
                }
            ...
            iRet = AW_MPI_VENC_SendFrame(iEncChn, &stFrameInfo, 60);
            if (iRet < 0) {
                aloge("Vipp2VencThread send one frame to video encoder failed!!\n");
                AW_MPI_VI_ReleaseFrame(iVippDev, iVippChn, &stFrameInfo);
                goto process;
            }

            while(g_bWaitVencDone != 1) {
                usleep(100);
            }
            g_bWaitVencDone = 0;
            AW_MPI_VI_ReleaseFrame(iVippDev, iVippChn, &stFrameInfo);
...
static int CreateVi2VencThread(SampleUVCContext *pConfig)
{
    ...
    if(PT_MJPEG == stVEncChnAttr.VeAttr.Type)
    {
        ...
        stVEncChnAttr.VeAttr.MaxKeyInterval = 1000 / pConfig->iCapFrameRate;
#if 1
        ...
        stVEncChnAttr.RcAttr.mAttrMjpegeFixQp.mQfactor    = pConfig->iEncQuality;
