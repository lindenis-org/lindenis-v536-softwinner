...
int extractStreamPacket(AUDIO_STREAM_S *pStreamInfo, FILE *fp, SampleADecConfig *pConf)
{
    ...
    int bs_sz = pkt_sz - pConf->mHeaderLen;
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // send stream to adec
        ret = AW_MPI_ADEC_SendStream(stContext.mADecChn, &nStreamInfo, 100);
        if (ret != SUCCESS)
        {
            alogw("send StreamId[%d] with 100ms timeout fail?!", nStreamInfo.mId);
        }
	...
	// Write pcm header
    alogd("decompress audio file and keep pcm file finish! then next write WAVE header!");
