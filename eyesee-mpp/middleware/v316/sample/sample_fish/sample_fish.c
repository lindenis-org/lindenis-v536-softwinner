...
static void *Loop_GetIseData(void *pArg)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            {
                char filename[125];
                sprintf(filename,"/%s/fish_ch%dport%d_%d.yuv",name, ISEGroup, ISEPort,i);
                FILE *fd = NULL;
...
int main(int argc, char *argv[])
{
    ...
    while (count != AutoTestCount)
    {
        ...
        for(j = 0; j < ISE_GROUP_INS_CNT; j++)
        {
            ...
            ret = initSampleFishFrameManager(&FisheyePara[j], 10);
            if(ret < 0)
            {
            ...
            ret = AW_MPI_ISE_SetISEFreq(FisheyePara[j].pISEGroupCap[0].ISE_Group, 576);
