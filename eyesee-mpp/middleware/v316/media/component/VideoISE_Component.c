...
ERRORTYPE DoVideoIseAddPort(PARAM_IN COMP_HANDLETYPE hComponent, int iseGrpId, int iseChnId, IseChnAttr *pIseChnAttr)
{
    ...
    if (ERR_ISE_UNEXIST == eRet)
    {
        ...
        for (i = 0; i < pEntry->mIseOutBufNum; i++)
        {
            ...
            {
                 // Y data, Phy & Vir Addr
                 pEntry->mpOutBuf->mIseOut.out_luma_mmu_Addr[iseChnId] = ion_allocMem(width * height);
                 ...
                 pEntry->mpOutBuf->mIseOut.out_chroma_u_mmu_Addr[iseChnId] = ion_allocMem(width * height / 2);
            ...
            else if (ISEMODE_TWO_ISE == pVideoIseData->mIseGrpAttr.iseMode)
            {
                ...
                if(pEntry->mIseChn == 0)
                {
                    pEntry->mpOutBuf->mStiOut.pano_luma_mmu_Addr =
...
ERRORTYPE VideoIseComponentInit(PARAM_IN COMP_HANDLETYPE hComponent)
{
    ...
    if (err || !pVideoIseData->thread_id) {
        eError = ERR_VENC_NOMEM;
        goto EXIT;
    }
