...
void check_soc_ft_fps(int *frame_mode)
{
	int fd;
	char buf[9];
	int ret = 0;
...
int video_init(struct isp_video_device *video)
{
	...
	csi_id = media_video_to_csi_id(video->entity);
	if ((csi_id < 0) || check_soc_ft_zone(csi_id)) {
		ISP_ERR("not support csi%d!!!\n", csi_id);
		return -ENOENT;
	}
