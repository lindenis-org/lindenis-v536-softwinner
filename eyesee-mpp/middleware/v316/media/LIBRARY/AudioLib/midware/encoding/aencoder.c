...
static int AudioEncInit(AudioEncoder *pEncoder, __audio_enc_inf_t * audio_inf, int encode_type)
{
	int i = 0;
	...
	audioEncData->gOutBufManager.buf_unused = FIFO_LEVEL;
	...
	audioEncData->PcmManager.uBufTotalLen	= BS_BUFFER_SIZE;
	audioEncData->PcmManager.pBufWritPtr		= audioEncData->PcmManager.pBufStart;
...
int InitializeAudioEncoder(AudioEncoder *pEncoder, AudioEncConfig *pConfig)
{
	...
	return AudioEncInit(pEncoder, &audioInfo, pConfig->nType);
}
