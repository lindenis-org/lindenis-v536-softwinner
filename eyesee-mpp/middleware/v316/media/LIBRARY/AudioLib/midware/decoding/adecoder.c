...
int64_t PlybkRequestPcmPts(AudioDecoder* pDecoder)
{
    ...
    {
        pts = (pAudioDecoder->pBsInFor->nDecodeMode == CDX_DECODER_MODE_RAWMUSIC ? (int64_t)pAudioDecoder->pBsInFor->NowPlayTime * 1000 - (int64_t)pAudioDecoder->ADCedarCtxA.nAOutBufferValidSize * (500000) / (pAudioDecoder->pBsInFor->out_channels * pAudioDecoder->pBsInFor->out_samplerate)
              : (pAudioDecoder->ADCedarCtxA.nAOutBufferValidSize ? pAudioDecoder->ADCedarCtxA.frmFifo.inFrames[pAudioDecoder->ADCedarCtxA.frmFifo.prefetchIdx].pts
...
AudioDecoder* CreateAudioDecoder(void)
{
    ...
    pthread_mutex_init(&pAudioDecoder->mutex_audiodec_thread, NULL);
