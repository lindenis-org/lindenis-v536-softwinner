...
{
    SavePcmFile_Stop = 0,
    SavePcmFile_InputData,
} SavePcmFileMessage;

static void PcmDataAddWaveHeader(SavePcmFile *pContext)
{
    struct WaveHeader{
        int riff_id;
        int riff_sz;
...
static int SavePcmFile_WriteLastData(SavePcmFile *pContext)
{
...
static void* SavePcmFileThread(void *pThreadData)
{
    SavePcmFile *pContext = (SavePcmFile*)pThreadData;
    message_t stMsg;
    ...
    PROCESS_MESSAGE:
        ...
        {
            alogv("SavePcmFile Thread get message cmd:%d", stMsg.command);
            if (SavePcmFile_Stop == stMsg.command)
            {
                //write last data to file.
                pthread_mutex_lock(&pContext->mBufferLock);
                SavePcmFile_WriteLastData(pContext);
            ...
            else if(SavePcmFile_InputData == stMsg.command)
            {
                //alogd("data input");
            }
            else
...
static int SavePcmFile_SendPcmData(void *pThiz, char *pData, int nSize, int nTimeOut)
{
    int result = 0;
    int bExit = 0;
    SavePcmFile *pContext = (SavePcmFile*)pThiz;
    ...
    if(nSize <= pContext->mBufferSize - pContext->mBufferValidLen)
    {
        ...
        if(pContext->mbWaitInputData!=0)
        {
            ...
            message_t msg;
            msg.command = SavePcmFile_InputData;
...
SavePcmFile* constructSavePcmFile(char *pFilePath, int nBufferSize, int bWavFlag, 
                unsigned int nSampleRate, unsigned int nChannelCount, unsigned int nSampleBitsPerChannel)
{
    int ret;
    SavePcmFile *pContext = (SavePcmFile*)malloc(sizeof(SavePcmFile));
    memset(pContext, 0, sizeof(SavePcmFile));
    ...
    ret = pthread_create(&pContext->mWriteFileThreadId, NULL, SavePcmFileThread, pContext);
    if(ret != 0)
    {
    ...
    {
        alogd("create SavePcmFile Thread[0x%x]", pContext->mWriteFileThreadId);
    }
    pContext->SendPcmData = SavePcmFile_SendPcmData;
    return pContext;
}

void destructSavePcmFile(SavePcmFile *pContext)
{
    message_t msg;
    msg.command = SavePcmFile_Stop;
    put_message(&pContext->mMessageQueue, &msg);
    void *pRet = NULL;
