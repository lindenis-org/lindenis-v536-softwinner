...
class GsensorManager
{
    public:
		...
		static GsensorManager* GetInstance();

        int8_t RunGsensorManager();

        int8_t StopGsensorManager();

        static void *GsensorManagerThread(void *context);
        static void *NormalModeImpactEventThread(void *context);
        static void *DealNormalAxisDataThread(void *context);
		...
		void IPReader_next();
		ssize_t IPReader_readEvent(input_event const** events);
		int64_t timevalToNano(timeval const& t);
		ssize_t IPReader_fill(int fd);

        int get_sysfs_input_attr(char *class_path,const char *attr, int *ivalue);
		...
		int getDevice(struct sensor_extend_t list[], struct sensor_info *info, char buf[], char classPath[], int number);
    ...
    private:
		...
		g_status seStatus[SUPPORT_SENSORS_NUMBER];
		sensor_t sSensorList[SUPPORT_SENSORS_NUMBER];
		int flag0;
		int flag1;
		int found;
		int sNumber;
