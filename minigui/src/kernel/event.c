...
BOOL kernel_GetLWEvent (int event, PLWEVENT lwe)
{
    ...
    if (event & IAL_MOUSEEVENT) {
       // fprintf(stderr, "kernel_GetLWEvent run here 00:\n");
        lwe->type = LWETYPE_MOUSE;
        if (kernel_RefreshCursor(&me->x, &me->y, &button)) {
    ...
    if (event & IAL_KEYEVENT) {
        ...
        lwe->type = LWETYPE_KEY;
        keystate = IAL_GetKeyboardState();
      //  fprintf(stderr, "kernel_GetLWEvent run here 11 nr_keys = %d:\n",nr_keys);
        for (i = 1; i < nr_keys; i++) {
         //  fprintf(stderr, "kernel_GetLWEvent run here 11 !oldkeystate[%d] = %d: keystate[i] = %s\n",i,!oldkeystate[i],keystate[i]);
            if(!oldkeystate[i] && keystate[i]) {
                 ke->event = KE_KEYDOWN;
                 ke_time =__mg_timer_counter;
                 ke->scancode = i;
        ...
        //fprintf(stderr, "kernel_GetLWEvent run here 11 nr_keys = %d  ; i = %d ,oldkey = %d:\n",nr_keys,i,olddownkey);
        if (i == nr_keys) {
            if (olddownkey == 0){
              //  fprintf(stderr, "kernel_GetLWEvent run here 22:\n");
                return 0;
            }
            ke->scancode = olddownkey;
            interval = __mg_timer_counter - ke_time;
            treat_longpress (ke, interval);
            if (ke->event == 0){
              //  fprintf(stderr, "kernel_GetLWEvent run here 33:\n");
                return 0;
            }
        }
    ...
    //fprintf(stderr, "kernel_GetLWEvent run here 44:\n");
    old_lwe.type = 0;
    return 0;
