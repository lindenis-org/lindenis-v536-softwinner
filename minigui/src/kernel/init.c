...
static void ParseEvent (PLWEVENT lwe)
{
    ...
    if (lwe->type == LWETYPE_TIMEOUT) {
        ...
        //fprintf(stderr, "ParseEvent Msg.wParam = %d , Msg.lParam = %d:\n",Msg.wParam,Msg.lParam);
        QueueDeskMessage (&Msg);
    }
        ...
        case ME_MOVED: {
            ...
            if (move_adjust) {//don't process the horizontal move
                if(deltY>0)
                    ...
                    direction = MOUSE_UP;
//                if(abs(deltY)>40){
//                    velocity = (int)(0.06f * powf((abs(40) + 7.5f), 2) - 2.5f + 0.5f);
//                }else{
//                    velocity = (int)(0.06f * powf((abs(deltY) + 7.5f), 2) - 2.5f + 0.5f);
//                }
                //velocity = (int)(0.06f * powf((abs(deltY) + 7.5f), 2) - 2.5f + 0.5f);
                //printf("####velocity%d,deltY%d####\n",velocity,deltY);
                velocity = abs(deltY);
