        ...
        HDC hdc, PITEMDATA pItem, RECT *rcItem, int nCols)
{
    ...
    if (psub->Image) {
        ...
        if (psub->dwFlags & LVIF_BITMAP) {
            ...
            //rect.left += 2;
            width = bmp->bmWidth;
            if (ph->mflags & LVCF_LEFTALIGN_IMAGE)
                rect.left +=0;
            else if (ph->mflags & LVCF_RIGHTALIGN_IMAGE)
                rect.left += (ph->width - width);
            else if (ph->mflags & LVCF_CENTERALIGN_IMAGE)
                rect.left += (ph->width - width)/2;
            FillBoxWithBitmap (hdc, rect.left, rect.top, width, height, bmp);
            ...
            if (ph->mflags & LVCF_LEFTALIGN_IMAGE)
                ...
                rect.left += (ph->width - width)/2;
            DrawIcon (hdc, rect.left, rect.top, width, height,
                    (HICON)psub->Image);
        ...
        case MSG_MOUSEMOVE:
            {
                int mouseX = LOSWORD (lParam);
                int mouseY = HISWORD (lParam);
                //fprintf(stderr,"habo---> listview MSG_MOUSEMOVE  x= %d  y= %d \n",mouseX,mouseY);
                move_flag = TRUE;
                ScreenToClient (hWnd, &mouseX, &mouseY);
...
#define WPARAM_MASK 0x400 //to get the direction of mouse move
                ...
                if (direction == MOUSE_UP) {
                    if(mouse_updown_flag == MOUSE_DOWN){
                        plvdata->cur_pos = ScrollViewCtrlProc(hWnd, SVM_GETCONTENTY, 0, 0);
                    }
                    plvdata->cur_pos += velocity;
                    mouse_updown_flag = MOUSE_UP;
                } else if (direction == MOUSE_DOWN) {
                    if(mouse_updown_flag == MOUSE_UP){
                        plvdata->cur_pos = ScrollViewCtrlProc(hWnd, SVM_GETCONTENTY, 0, 0);
                    }
                    plvdata->cur_pos -= velocity;
                    mouse_updown_flag = MOUSE_DOWN;
                }
                ...
                if (plvdata->cur_pos < 0) plvdata->cur_pos = 0;
                 //printf("####mouseX%d,mouseY%d,velocity%d,plvdata->cur_pos%d####\\n",mouseX,mouseY,velocity,plvdata->cur_pos);
                //return ScrollViewCtrlProc (hWnd, MSG_VSCROLL, SB_THUMBTRACK, plvdata->cur_pos);
                return ScrollViewCtrlProc (hWnd, MSG_VSCROLL, SB_THUMBTRACK_LISTVIEW, plvdata->cur_pos);
        ...
        case MSG_LBUTTONDOWN:
            {
                ...
                move_distance = mouseY+120;//处理由于屏幕上下偏移了60问题
                #else
                move_distance = mouseY;
                #endif
                //fprintf(stderr,"habo---> listview MSG_LBUTTONDOWN  x= %d  y= %d move_distance= %d\n",mouseX,mouseY,move_distance);
                RECT rect, rcClient;
                PLSTHDR p1;
                PITEMDATA pi;
        ...
        case MSG_LBUTTONUP:
            {
                ...
                int mouseY = HISWORD (lParam);
                move_distance -= mouseY;
                //fprintf(stderr,"habo---> listview MSG_LBUTTONUP  x= %d  y= %d move_distance= %d\n",mouseX,mouseY,move_distance);
                LVNM_NORMAL lvnm;
